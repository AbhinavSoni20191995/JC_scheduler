...
def migrate2(db):
    ...
    if "priority" not in cols:
        # extra turns owed to this person next round (from missed presentations)
        db.execute("ALTER TABLE participants ADD COLUMN priority INTEGER NOT NULL DEFAULT 0")
    db.commit()
...
def generate_schedule(db):
    ...
    # Each track walks its own boosted sequence: priority people appear at the
    # front (extra copies = extra turns), then the normal round-robin order,
    # repeated enough times to cover the horizon.
    def make_sequence(base_ids):
        extra = []
        for p in sorted(people, key=lambda x: -x["priority"]):
            for _ in range(p["priority"]):
                extra.append(p["id"])
        seq = extra + base_ids
        # repeat so we never run short over the horizon
        reps = (horizon * 2) // max(1, len(seq)) + 2
        return seq * reps

    def pick_from(seq, pos, role, avoid_id):
        """Advance through seq from pos to the next eligible speaker,
        avoiding double duty when possible. Returns (pid, next_pos)."""
        for relax in (False, True):
            scan = pos
            for _ in range(len(seq)):
                pid = seq[scan % len(seq)]
                scan += 1
                if eligible(pid, role) and (relax or n == 1 or pid != avoid_id):
                    return pid, scan
        return None, pos + 1

    def start_position(role, seq, default_pid_index):
        """Where in the boosted sequence this track should begin."""
        ...
        if target_pid is None:
            # continue after the last assigned speaker of this role
            last = db.execute(
            ...
            if last and last["speaker_id"] in ids:
                # start just after them in the base order (skip the boost block,
                # which is only for a fresh round)
                base_start = seq.index(last["speaker_id"], len(seq) - len(ids)) \
                    if last["speaker_id"] in seq[len(seq)-len(ids):] else None
                if base_start is not None:
                    return base_start + 1
        if target_pid is not None and target_pid in seq:
            return seq.index(target_pid)
        return default_pid_index

    jc_seq = make_sequence(ids)
    pr_seq = make_sequence(ids)
    jc_pos = start_position("jc", jc_seq, 0)
    pr_pos = start_position("progress", pr_seq,
                            (len(jc_seq) - len(ids)) + max(1, n // 2))
    ...
    for _ in range(horizon):
        ...
        if iso in holidays:
            ...
            if row_jc is None and (iso, "jc") not in existing:
                jc_speaker, jc_pos = pick_from(jc_seq, jc_pos, "jc", pr_speaker)
            ...
            if row_pr is None and (iso, "progress") not in existing:
                pr_speaker, pr_pos = pick_from(pr_seq, pr_pos, "progress", jc_speaker)
    ...
    def rows_for(role):
        ...
        for p in people:
            ...
            rec.update({"id": p["id"], "name": p["name"],
                        "once_only": p["once_only"], "priority": p["priority"],
                        "active": p["active"]})
            out.append(rec)
...
def apply_semester_tally(db, boost_multiplier=2):
    """End-of-round fairness pass. Anyone active (not once-only) who did NOT
    present at all in a track — i.e. presented 0 and has no upcoming slot —
    gets priority so they present more, and sooner, next round. People who
    presented get priority reset to 0. Returns a summary list."""
    stats = compute_stats(db)
    summary = []
    # build per-person shortfall across both tracks
    shortfall = {}
    for role in ("jc", "progress"):
        for r in stats[role]:
            if r["once_only"] or not r["active"]:
                continue
            missed = (r["presented"] == 0 and r["upcoming"] == 0)
            if missed:
                shortfall[r["id"]] = shortfall.get(r["id"], 0) + 1
    # cancellations also add a small boost (they were meant to present)
    for r in stats["both"]:
        if r["once_only"] or not r["active"]:
            continue
        if r["cancelled"] > 0 and r["id"] not in shortfall:
            shortfall[r["id"]] = shortfall.get(r["id"], 0) + 1

    for p in db.execute("SELECT * FROM participants WHERE active=1"):
        pid = p["id"]
        if p["once_only"]:
            continue
        new_priority = shortfall.get(pid, 0) * (boost_multiplier - 1) if pid in shortfall else 0
        # people who missed get extra turns; everyone else resets to 0
        db.execute("UPDATE participants SET priority=? WHERE id=?", (new_priority, pid))
        if new_priority:
            summary.append({"name": p["name"], "priority": new_priority})
    db.commit()
    # order the suggested rotation: highest priority first, then existing order
    ordered = db.execute(
        "SELECT name, priority FROM participants WHERE active=1 "
        "ORDER BY priority DESC, position, id").fetchall()
...
@admin_required
def admin_semester():
    """End-of-round tally: boost under-presenters for the next round."""
    db = get_db()
    summary, ordered = apply_semester_tally(db)
    # optionally set a schedule start date for the new round
    start = request.form.get("start_date", "").strip()
    if start:
        set_setting("schedule_start", start)
    n = len(summary)
    if n:
        names = ", ".join(f"{s['name']} (+{s['priority']})" for s in summary)
        flash(f"New round prepared. Boosted {n} person(s): {names}. "
              f"Generate or rebuild the schedule to apply.")
    else:
        flash("New round prepared — everyone presented last round, no boosts needed.")
...
@admin_required
def admin_priority():
    """Manually adjust one person's priority (extra turns) from the stats page."""
    db = get_db()
    pid = request.form.get("id", type=int)
    delta = request.form.get("delta", type=int) or 0
    db.execute("UPDATE participants SET priority = MAX(priority + ?, 0) WHERE id=?",
               (delta, pid))
    db.commit()
