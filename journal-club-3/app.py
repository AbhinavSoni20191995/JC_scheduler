...
def compute_stats(db):
    ...
    for s in db.execute("SELECT speaker_id, original_speaker_id, role, status "
...
def apply_semester_tally(db):
    ...
    active (non once-only) person presented THAT track this round, compares to
    that track's average, and sets that track's priority:
    ...
    and more progress next round, independently. 'This round' counts presented +
    upcoming assigned sessions. Returns (summary, suggested_order)."""
    import math
...
def admin_semester():
    ...
    db = get_db()
    summary, ordered = apply_semester_tally(db)
    ...
    if summary:
        ...
        flash("New round prepared, balanced per track: "
