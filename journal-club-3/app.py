...
def take_over(db, sess, volunteer_id):
    ...
    send_email(
        ...
        f"Friday {sess['day']} from {sess['speaker_name']}.{swapped}{double}\n\n"
        f"Schedule: {BASE_URL}",
        db=db)
    return True
...
def index():
    ...
    past_rows = db.execute(
        "SELECT s.*, p.name AS speaker_name FROM sessions s "
        "LEFT JOIN participants p ON p.id = s.speaker_id "
        "WHERE s.day < ? ORDER BY s.day DESC LIMIT 50", (today,)).fetchall()
    upcoming = grouped(up_rows)
    past = grouped(past_rows)

    next_day = None
    for d, pair in upcoming.items():
        if any(x["status"] != "cancelled" for x in pair.values()):
            next_day = d
            break

    return render_template("index.html", upcoming=upcoming, past=past,
                           next_day=next_day, lab_name=setting("lab_name"))
